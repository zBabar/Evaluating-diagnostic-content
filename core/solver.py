...
class CaptioningSolver(object):
    ...
    def train(self):
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            ...
            _, _, generated_captions = self.model.build_sampler(max_len=40)

        # train op
        with tf.variable_scope(tf.get_variable_scope(), reuse=False):
            early_optimizer = self.optimizer(learning_rate=self.learning_rate)
            late_optimizer = self.optimizer(learning_rate=self.learning_rate/10)
            ...
            for e in range(self.n_epochs):
                ...
                for i in range(n_iters_per_epoch):
                    ...
                    if(e<=30):
                        ...
                        _, l = sess.run([train_op1, loss], feed_dict)
                    curr_loss += l
                    ...
                    if (i+1) % self.print_every == 0:
                        ...
                        temp=image_idxs == image_idxs_batch[0]
                ...
                if self.print_bleu:
                    all_gen_cap = np.ndarray((val_features.shape[0], 40))
